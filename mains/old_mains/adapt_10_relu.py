...
save_path = "../data_relu_adapt_10_full"
...
for i in tqdm.tqdm(range(20)): 
    ...
    with open(save_path + '/trdata_run_{}_{}2.pickle'.format(i, ext), 'wb') as f:
        pickle.dump([tr.error_train, tr.error_test], f, protocol=pickle.HIGHEST_PROTOCOL)
        f.close()

    mutual_inf = MI(tr.hidden_activations, act_full_loader,act=activation, num_of_bins=10)
    MI_XH, MI_YH = mutual_inf.get_MI(method="adaptive")
    with open(save_path + '/MI_XH_MI_YH_run_{}_{}2.pickle'.format(i, ext), 'wb') as f:
