...
for i in tqdm.tqdm(range(50)): 
    ...
    optimizer = optim.Adam(ib_model.parameters(), lr=0.001)
    tr = Trainer(config, ib_model, optimizer, device)
    tr.train(train_loader, test_loader, act_loaders)
    with open('./data_tanh_full/trdata_run_{}_{}2.pickle'.format(i, ext), 'wb') as f:
    ...
    with open('./data_tanh_full/MI_XH_MI_YH_run_{}_{}2.pickle'.format(i, ext), 'wb') as f:
