...
for i in tqdm.tqdm(range(50)): 
    ...
    with open(save_path + '/trdata_run_{}_{}.pickle'.format(i, ext), 'wb') as f:
        ...
        f.close()
    mutual_inf = MI(tr.hidden_activations, act_full_loader,act=activation, num_of_bins=100)
    MI_XH, MI_YH = mutual_inf.get_MI()

    mutual_inf2 = MI(tr.hidden_activations, act_full_loader,act=activation, num_of_bins=300)
    MI_XH2, MI_YH2 = mutual_inf2.get_MI()

    with open(save_path + '/MI_XH_MI_YH_run_{}_{}.pickle'.format(i, ext), 'wb') as f:
        pickle.dump([MI_XH, MI_YH], f, protocol=pickle.HIGHEST_PROTOCOL)
        f.close()

    with open(save_path + '/MI_XH_MI_YH_run_{}_{}200bins.pickle'.format(i, ext), 'wb') as f:
